...
```{r data_clean, echo=TRUE, warning=FALSE}
...
```{r generate_word_cloud, echo=FALSE}
#generate word cloud
plot_word_cloud = function(data, min_freq, name) {
  set.seed(1234)
  wordcloud(words = data$word, freq = data$freq, min.freq = min_freq, main = paste0("Word Cloud for ", name),
          max.words=100, random.order=FALSE, rot.per=0.40, 
          colors=brewer.pal(8, "Dark2"))
}

twitter_cloud = plot_word_cloud(twitter_count, 20000, "Twitter")
blog_cloud = plot_word_cloud(blogs_count, 20000, "Blogs")
news_cloud = plot_word_cloud(news_count, 20000, "News")
```
...
```
##### Twitter String Summary
```{r echo=FALSE}
string.summary(twitter_str)
```
##### Blogs String Summary
```{r echo=FALSE}
